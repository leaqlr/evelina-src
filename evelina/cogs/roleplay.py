...
class Roleplay(Cog):
    ...
    @command(name="kiss", usage="kiss accurs", description="Kiss a member")
    async def kiss(self, ctx: EvelinaContext, *, member: Member):
        if member == ctx.author:
            return await ctx.send_warning(f"You can't kiss yourself silly")
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/kiss?key={config.EVELINA}")
    ...
    @command(name="lick", aliases=["slurp"], usage="lick accurs", description="Lick a member")
    async def lick(self, ctx: EvelinaContext, *, member: Member):
        ...
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/lick?key={config.EVELINA}")
    ...
    @command(name="fuck", usage="fuck accurs", brief="NSFW Channel", description="Fuck a member")
    @nsfw_channel()
    async def fuck(self, ctx: EvelinaContext, *, member: Member):
        if member == ctx.author:
            return await ctx.send_warning("You can't fuck yourself silly")
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/fuck?key={config.EVELINA}")
    ...
    @command(name="anal", usage="fuck accurs", brief="NSFW Channel", description="Fuck a member anal")
    @nsfw_channel()
    async def anal(self, ctx: EvelinaContext, *, member: Member):
        if member == ctx.author:
            return await ctx.send_warning("You can't fuck yourself anal silly")
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/anal?key={config.EVELINA}")
    ...
    @command(name="blowjob", usage="blowjob accurs", brief="NSFW Channel", description="Blowjob a member")
    @nsfw_channel()
    async def blowjob(self, ctx: EvelinaContext, *, member: Member):
        if member == ctx.author:
            return await ctx.send_warning("You can't blowjob yourself silly")
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/blowjob?key={config.EVELINA}")
    ...
    @command(name="cum", usage="cum accurs", brief="NSFW Channel", description="Cum on a member")
    @nsfw_channel()
    async def cum(self, ctx: EvelinaContext, *, member: Member):
        if member == ctx.author:
            return await ctx.send_warning("You can't cum on yourself silly")
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/cum?key={config.EVELINA}")
    ...
    @command(name="pussylick", usage="pussylick accurs", brief="NSFW Channel", description="Pussylick a member")
    @nsfw_channel()
    async def pussylick(self, ctx: EvelinaContext, *, member: Member):
        ...
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/pussylick?key={config.EVELINA}")
    ...
    @threesome.command(name="fff", usage="threesome fff accurs visics", brief="NSFW Channel", description="Threesome with only girls")
    @nsfw_channel()
    async def threesome_fff(self, ctx: EvelinaContext, member: Member, partner: Member):
        ...
        if partner == ctx.author:
            return await ctx.send_warning("You can't threesome with yourself silly")
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/threesome_fff?key={config.EVELINA}")
        embed = Embed(color=colors.NEUTRAL, description=f"*Oh yeah!* **{ctx.author.name}** started a threesome with **{member.name}** & **{partner.name}**")
        embed.set_image(url=result["url"])
        return await ctx.reply(embed=embed)
    
    @threesome.command(name="ffm", usage="threesome ffm accurs visics", brief="NSFW Channel", description="Threesome with two girls & one boy")
    @nsfw_channel()
    async def threesome_ffm(self, ctx: EvelinaContext, member: Member, partner: Member):
        ...
        if partner == ctx.author:
            return await ctx.send_warning("You can't threesome with yourself silly")
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/threesome_ffm?key={config.EVELINA}")
        embed = Embed(color=colors.NEUTRAL, description=f"*Oh yeah!* **{ctx.author.name}** started a threesome with **{member.name}** & **{partner.name}**")
        embed.set_image(url=result["url"])
        return await ctx.reply(embed=embed)
    
    @threesome.command(name="fmm", usage="threesome fmm accurs visics", brief="NSFW Channel", description="Threesome with one girl & two boys")
    @nsfw_channel()
    async def threesome_fmm(self, ctx: EvelinaContext, member: Member, partner: Member):
        ...
        if partner == ctx.author:
            return await ctx.send_warning("You can't threesomew with yourself silly")
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/threesome_fmm?key={config.EVELINA}")
        embed = Embed(color=colors.NEUTRAL, description=f"*Oh yeah!* **{ctx.author.name}** started a threesome with **{member.name}** & **{partner.name}**")
        embed.set_image(url=result["url"])
        return await ctx.reply(embed=embed)
    
    @command(name="pinch",usage="pinch accurs", description="Pinch a member")
    async def pinch(self, ctx: EvelinaContext, *, member: Member):
        if member == ctx.author:
            return await ctx.send_warning("You can't pinch yourself silly")
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/pinch?key={config.EVELINA}")
    ...
    @command(name="cuddle", usage="cuddle accurs", description="Cuddle a member")
    async def cuddle(self, ctx: EvelinaContext, *, member: Member):
        if member == ctx.author:
            return await ctx.send_warning("You can't cuddle yourself silly")
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/cuddle?key={config.EVELINA}")
    ...
    @command(name="hug", usage="hug accurs", description="Hug a member")
    async def hug(self, ctx: EvelinaContext, *, member: Member):
        if member == ctx.author:
            return await ctx.send_warning("You can't hug yourself silly")
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/hug?key={config.EVELINA}")
    ...
    @command(name="pat", usage="pat accurs", description="Pat a member")
    async def pat(self, ctx: EvelinaContext, *, member: Member):
        if member == ctx.author:
            return await ctx.send_warning("You can't pat yourself silly")
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/pat?key={config.EVELINA}")
    ...
    @command(name="slap", usage="slap accurs", description="Slap a member")
    async def slap(self, ctx: EvelinaContext, *, member: Member):
        if member == ctx.author:
            return await ctx.send_warning("You can't slap yourself silly")
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/slap?key={config.EVELINA}")
    ...
    async def laugh(self, ctx: EvelinaContext):
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/laugh?key={config.EVELINA}")
    ...
    async def cry(self, ctx: EvelinaContext):
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/cry?key={config.EVELINA}")
        embed = Embed(color=colors.NEUTRAL, description=f"**{ctx.author.name}** cries")
        embed.set_image(url=result["url"])
        return await ctx.reply(embed=embed)
    
    @command(name="stfu", usage="stfu accurs", description="Tell a member to shut up")
    ...
    @command(name="ship", usage="ship accurs curet", description="Check the ship rate between you and a member")
    ...
    @command(name="marry", usage="marry accurs", description="Marry a member")
    ...
    @command(name="marriage", usage="marriage accurs", description="View your marriage or from a given user")
    ...
    @command(name="married", usage="married accurs curet", description="Create a marriage certificate")
