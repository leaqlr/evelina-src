...
from discord import TextChannel, Embed, File, Member, Interaction, User, Thread, ForumChannel, Guild
...
class Utility(Cog):
    ...
    async def fyp(self, ctx: EvelinaContext):
        ...
        async with ctx.channel.typing():
            for attempt in range(3):
                x = await self.bot.session.get_json("https://api.yourbotbot/tiktok/fyp", params={"key": config.EVELINA})
    ...
    @avatarhistory.command(name="view", usage="avatarhistory view accurs")
    ...
    @command(aliases=["firstmsg"], usage="firstmessage #general accurs")
    ...
    @command(aliases=["av"], usage="avatar accurs")
    ...
    @command(aliases=["sav", "savatar"], usage="serveravatar accurs")
    ...
    @command(aliases=["sbanner"], usage="serverbanner accurs")
    ...
    @command(name="names", aliases=["pastusernames", "usernames", "oldnames", "pastnames"], usage="names accurs")
    ...
    @command(name="permissions", aliases=["perms"], usage="permissions accurs")
    ...
    @command(aliases=["user", "ui", "whois"], usage="userinfo accurs")
    ...
    @command(name="info", usage="info accurs")
    ...
    @command(name="devices", usage="devices accurs")
    ...
    async def weather(self, ctx: EvelinaContext, *, city: str):
        ...
        try:
            data = await self.bot.session.get_json(f"https://api.yourbotbot/weather?city={city}&key={config.EVELINA}")
            if 'message' in data:
    ...
    @command(brief="manage messages", aliases=["pic"], usage="picperms accurs")
    @has_guild_permissions(manage_messages=True)
    ...
    @command(brief="manage messages", usage="stickerperms accurs")
    @has_guild_permissions(manage_messages=True)
    @bot_has_guild_permissions(manage_channels=True)
    ...
    @command(brief="manage messages", usage="emojiperms accurs")
    @has_guild_permissions(manage_messages=True)
    @bot_has_guild_permissions(manage_channels=True)
    ...
    @group(name="language", aliases=["lang"], usage="language accurs", invoke_without_command=True, case_insensitive=True)
    ...
    async def spotify_old_track(self, ctx: EvelinaContext, *, query: str):
        ...
        data = await self.bot.session.get_json(f"https://api.yourbotbot/spotify/track?q={query}&key={config.EVELINA}")
        if 'message' in data:
    ...
    async def lyrics(self, ctx: EvelinaContext, *, song: str):
        try:
            song_data = lyricsgenius.Genius(config.GENIUS).search_song(song)
            if song_data is None:
                await ctx.send_warning(f'No lyrics found for **{song}**')
                return
            ...
            lyrics = '\n'.join(lyrics_lines[1:])
            cover_url = song_data.song_art_image_url
            strophen = re.split(r'\n{2,}', lyrics)
            embeds = []
            temp_page = ""
            for strophe in strophen:
                if len(temp_page) + len(strophe) > 1000:
                    embed = Embed(title=f"{title} by {artist}", description=temp_page.strip(), color=colors.NEUTRAL, url=song_url)
                    embed.set_thumbnail(url=cover_url if cover_url else None)
            ...
            for i, embed in enumerate(embeds):
                embed.set_footer(text=embed.footer.text.format(len(embeds)))
            await ctx.paginator(embeds)
        except Exception:
            await ctx.send_warning(f"An error occurred while trying to fetch the lyrics for **{song}**")
    ...
    @messages.command(name="user", usage="messages user accurs")
    ...
    @command(name="sync", usage="sync accurs", cooldown=60)
    ...
    @activity.command(name="user", usage="activity user accurs 30d")
