...
class Info(Cog):
    ...
    async def botinfo(self, ctx: EvelinaContext):
        ...
        cache_key = "botinfo_stats"
        cached_stats = await self.bot.cache.get(cache_key)
        
        if cached_stats:
            total_users, human_users, bot_users, text_channels, voice_channels, categorie_channels = cached_stats
        else:
            total_users = sum(g.member_count for g in self.bot.guilds if g.member_count is not None)
            human_users = sum(1 for g in self.bot.guilds for m in g.members if not m.bot)
            bot_users = sum(1 for g in self.bot.guilds for m in g.members if m.bot)
            ...
            stats = (total_users, human_users, bot_users, text_channels, voice_channels, categorie_channels)
            await self.bot.cache.set(cache_key, stats, expiration=3600) 
        ...
        embed = (
            Embed(
                ...
                description=(
                    ...
                    f"Used by **{total_users:,}** members in "
                    f"**{len(self.bot.guilds):,}** servers on **{len(self.bot.shards):,}** shards"
                )
            )
            .set_author(
    ...
    async def shards(self, ctx: EvelinaContext):
        ...
        for shard in self.bot.shards:
            ...
            shard_info = {
                "shard_id": shard,
                "ping": round(self.bot.shards.get(shard).latency * 1000),
                "guilds": len(guilds),
                "users": users,
                "is_current": shard == current_guild_shard_id
            }
        ...
        for i in range(0, len(shards_data), entries_per_page):
            ...
            embed = Embed(color=colors.NEUTRAL, title=f"Total shards ({shard_count})")
