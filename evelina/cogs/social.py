...
class Social(Cog):
    ...
    async def github(self, ctx: EvelinaContext, username: str):
        ...
        data = await self.bot.session.get_json(f"https://api.yourbotbot/github/user?username={username}&key={config.EVELINA}")
        if 'message' in data:
    ...
    async def roblox(self, ctx: EvelinaContext, username: str):
        ...
        data = await self.bot.session.get_json(f"https://api.yourbotbot/roblox/user?username={username}&key={config.EVELINA}")
        if 'message' in data:
    ...
    async def gunslink(self, ctx: EvelinaContext, uid: int):
        ...
        if check:
            if int(check['uid']) != uid:
                old_uid_data = await self.bot.session.get_json("https://api.yourbotbot/guns/uid", params={"id": check['uid'], "key": config.EVELINA})
    ...
    async def gunslist(self, ctx: EvelinaContext):
        ...
        async def fetch_user_data(entry):
            user_data = await self.bot.session.get_json("https://api.yourbotbot/guns/uid", params={"id": entry['uid'], "key": config.EVELINA})
    ...
    async def snapchatstory(self, ctx: EvelinaContext, username: str):
        ...
        try:
            async with ctx.typing():
                data = await self.bot.session.get_json("https://api.yourbotbot/snapchat/story", params={"username": username.lower(), "key": config.EVELINA})
    ...
    async def minecraft(self, ctx: EvelinaContext, username: str):
        ...
        data, status = await self.bot.session.get_json(f"https://api.yourbotbot/minecraft?username={username}&key={config.EVELINA}", return_status=True)
        if status != 200:
            return await ctx.send_warning(f"Couldn't get information about **{username}**")
    ...
    async def steam(self, ctx: EvelinaContext, steamid: int):
        ...
        data = await self.bot.session.get_json(f"https://api.yourbotbot/steam/user?id={steamid}&key={config.EVELINA}")
        if 'message' in data:
