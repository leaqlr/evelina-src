...
class Selfbot(Cog):
    ...
    async def tiktok_video(self, interaction: Interaction, url: str):
        ...
        try:
            x = await self.bot.session.get_json("https://api.yourbotbot/tiktok/media", params={"url": url, "key": config.EVELINA})
        except Exception:
    ...
    async def tiktok_user(self, interaction: Interaction, username: str):
        ...
        if await interaction.client.db.fetchrow("SELECT reason FROM blacklist_user WHERE user_id = $1", interaction.user.id):
            embed = Embed(color=colors.WARNING, description=f"{emojis.WARNING} {interaction.user.mention}: You are blacklisted from using **{interaction.client.user.mention}**")
            return await interaction.followup.send(embed=embed, ephemeral=True)
        data = await self.bot.session.get_json(f"https://api.yourbotbot/tiktok/user?username={username.lower()}&key={config.EVELINA}") 
        if 'message' in data:
        ...
        embed = (
            discord.Embed(color=colors.TIKTOK, title=f"{data['full_name']} (@{data['username']})", url=f"https://tiktok.com/@{data['username']}/", description=data['bio'])
            ...
            .add_field(name="Hearts", value=f"{data['hearts']:,}")
    ...
    async def process_instagram(self, interaction: Interaction, url: str):
        api_url = "https://api.yourbotbot/instagram/media"
    ...
    async def instagram_post(self, interaction: Interaction, url: str):
        ...
        if await interaction.client.db.fetchrow("SELECT reason FROM blacklist_user WHERE user_id = $1", interaction.user.id):
            ...
            return await interaction.followup.send(embed=embed, ephemeral=True)
        try:
            data = await self.bot.session.get_json("https://api.yourbotbot/instagram/post", params={"url": url, "key": config.EVELINA})
    ...
    async def instagram_posts(self, interaction: Interaction, username: str):
        ...
        if await interaction.client.db.fetchrow("SELECT reason FROM blacklist_user WHERE user_id = $1", interaction.user.id):
            ...
            return await interaction.followup.send(embed=embed, ephemeral=True)
        try:
            data = await self.bot.session.get_json("https://api.yourbotbot/instagram/posts", params={"username": username, "key": config.EVELINA})
    ...
    async def instagram_story(self, interaction: Interaction, username: str):
        ...
        if await interaction.client.db.fetchrow("SELECT reason FROM blacklist_user WHERE user_id = $1", interaction.user.id):
            ...
            return await interaction.followup.send(embed=embed, ephemeral=True)
        try:
            data = await self.bot.session.get_json("https://api.yourbotbot/instagram/story", params={"username": username.lower(), "key": config.EVELINA})
    ...
    async def instagram_user(self, interaction: Interaction, username: str):
        ...
        if await interaction.client.db.fetchrow("SELECT reason FROM blacklist_user WHERE user_id = $1", interaction.user.id):
            embed = Embed(color=colors.WARNING, description=f"{emojis.WARNING} {interaction.user.mention}: You are blacklisted from using **{interaction.client.user.mention}**")
            return await interaction.followup.send(embed=embed, ephemeral=True)
        data = await self.bot.session.get_json(f"https://api.yourbotbot/instagram/user?username={username.lower()}&key={config.EVELINA}")
        if 'message' in data:
            return await interaction.followup.send(embed=Embed(color=colors.WARNING, description=f"{emojis.WARNING} {interaction.user.mention}: Couldn't get information about **{username}**"), ephemeral=True)
        embed = (
    ...
    async def process_snapchat(self, interaction: Interaction, url: str):
        api_url = "https://api.yourbotbot/snapchat/media"
        x = await self.bot.session.get_json(api_url, params={"url": url, "key": config.EVELINA})
        if "video" in x and x["video"].get("video"):
            video = x["video"]["video"]
    ...
    async def snapchat_user(self, interaction: Interaction, username: str):
        ...
        if await interaction.client.db.fetchrow("SELECT reason FROM blacklist_user WHERE user_id = $1", interaction.user.id):
            embed = Embed(color=colors.WARNING, description=f"{emojis.WARNING} {interaction.user.mention}: You are blacklisted from using **{interaction.client.user.mention}**")
            return await interaction.followup.send(embed=embed, ephemeral=True)
        data = await self.bot.session.get_json("https://api.yourbotbot/snapchat/user", params={"username": username, "key": "8F6qVxN55aoODT0FRh16pydP"})
        if 'message' in data:
            return await interaction.followup.send(embed=Embed(color=colors.WARNING, description=f"{emojis.WARNING} {interaction.user.mention}: Couldn't get information about **{username}**"), ephemeral=True)
        embed = (
    ...
    async def process_twitter(self, interaction: Interaction, url: str):
        api_url = "https://api.yourbotbot/twitter/media"
        x = await self.bot.session.get_json(api_url, params={"url": url, "key": config.EVELINA})
        if "video" in x and x["video"].get("video"):
            video = x["video"]["video"]
    ...
    async def twitter_user(self, interaction: Interaction, username: str):
        ...
        if await interaction.client.db.fetchrow("SELECT reason FROM blacklist_user WHERE user_id = $1", interaction.user.id):
            embed = Embed(color=colors.WARNING, description=f"{emojis.WARNING} {interaction.user.mention}: You are blacklisted from using **{interaction.client.user.mention}**")
            return await interaction.followup.send(embed=embed, ephemeral=True)  
        data = await self.bot.session.get_json(f"https://api.yourbotbot/twitter/user?username={username.lower()}&key={config.EVELINA}")
        if 'message' in data:
            return await interaction.followup.send(embed=Embed(color=colors.WARNING, description=f"{emojis.WARNING} {interaction.user.mention}: Couldn't get information about **{username}**"), ephemeral=True)
    ...
    async def pinterest_video(self, interaction: Interaction, url: str):
        ...
        if cooldown:
            return await interaction.followup.send(embed=Embed(color=colors.WARNING, description=f"{emojis.WARNING} {interaction.user.mention}: You are on cooldown. Please try again later"), ephemeral=True)
        try:
            x = await self.bot.session.get_json("https://api.yourbotbot/pinterest/media", params={"url": url, "key": config.EVELINA})
            if x['type'] == 'image':
    ...
    async def crypto(self, interaction: Interaction, amount: float, from_currency: str, to_currency: str) -> None:
        ...
        if await interaction.client.db.fetchrow("SELECT reason FROM blacklist_user WHERE user_id = $1", interaction.user.id):
            ...
            return await interaction.followup.send(embed=embed, ephemeral=True)
        try:
            data = await self.bot.session.get_json(f"https://api.yourbotbot/crypto?amount={amount}&from={from_currency}&to={to_currency}&key={config.EVELINA}")
    ...
    async def avatarhistory(self, interaction: Interaction, *, user: User = None) -> None:
        ...
        base_url = "https://cdn.yourbot.bot/avatars/"
    ...
    async def guns_user(self, interaction: Interaction, *, username: str):
        ...
        if await interaction.client.db.fetchrow("SELECT reason FROM blacklist_user WHERE user_id = $1", interaction.user.id):
            embed = Embed(color=colors.WARNING, description=f"{emojis.WARNING} {interaction.user.mention}: You are blacklisted from using **{interaction.client.user.mention}**")
            return await interaction.followup.send(embed=embed, ephemeral=True)
        data = await self.bot.session.get_json("https://api.yourbotbot/guns/user", params={"username": username, "key": config.EVELINA})
        if 'message' in data:
    ...
    async def guns_uid(self, interaction: Interaction, *, uid: str):
        ...
        if await interaction.client.db.fetchrow("SELECT reason FROM blacklist_user WHERE user_id = $1", interaction.user.id):
            embed = Embed(color=colors.WARNING, description=f"{emojis.WARNING} {interaction.user.mention}: You are blacklisted from using **{interaction.client.user.mention}**")
            return await interaction.followup.send(embed=embed, ephemeral=True)
        data = await self.bot.session.get_json("https://api.yourbotbot/guns/uid", params={"id": uid, "key": config.EVELINA})
        if 'message' in data:
...
async def user_avatar(interaction: discord.Interaction, member: discord.Member):
    embed = discord.Embed(color=await interaction.client.misc.dominant_color(member.avatar.url if member.avatar else member.default_avatar.url), title=f"{member.name}'s avatar", url=member.avatar.url if member.avatar else member.default_avatar.url)
...
async def user_banner(interaction: discord.Interaction, member: discord.Member):
    ...
    banner = member.banner.url
    embed = discord.Embed(color=await interaction.client.misc.dominant_color(banner), title=f"{member.name}'s banner", url=banner)
...
async def user_info(interaction: discord.Interaction, member: discord.Member):
        ...
        async def lastfm(mem: discord.Member):
            lastfm = await interaction.client.db.fetchval("SELECT username FROM lastfm WHERE user_id = $1", mem.id)
            if lastfm:
                a = await asyncio.wait_for(Handler(interaction.client, config.API.LASTFM).get_tracks_recent(lastfm, 1), timeout=15)
            ...
            return ""
        badges = await bot.misc.get_badges(member)
        description = ""
        if badges:
            description += f"### {badges}\n"
...
@bot.tree.context_menu(name="Voice Permit")
async def voice_permit(interaction: discord.Interaction, member: discord.Member):
    ...
    if check is not None:
        ...
        if channel is None:
            embed = discord.Embed(color=colors.WARNING, description=f"{emojis.WARNING} {interaction.user.mention}: The voice master channel could not be found")
            return await interaction.response.send_message(embed=embed, ephemeral=True)
        category = channel.category
        if category is None:
            embed = discord.Embed(color=colors.WARNING, description=f"{emojis.WARNING} {interaction.user.mention}: The voice master channel does not have a category")
            return await interaction.response.send_message(embed=embed, ephemeral=True)
        if interaction.user.voice is None or interaction.user.voice.channel is None:
            embed = discord.Embed(color=colors.WARNING, description=f"{emojis.WARNING} {interaction.user.mention}: You are not in a voice channel")
            return await interaction.response.send_message(embed=embed, ephemeral=True)
        elif interaction.user.voice.channel.category != category:
            embed = discord.Embed(color=colors.WARNING, description=f"{emojis.WARNING} {interaction.user.mention}: You are not in a voice channel created by the bot")
            return await interaction.response.send_message(embed=embed, ephemeral=True)
        check = await interaction.client.db.fetchrow("SELECT * FROM voicemaster_channels WHERE voice = $1 AND user_id = $2", interaction.user.voice.channel.id, interaction.user.id)
        if check is None:
            embed = discord.Embed(color=colors.WARNING, description=f"{emojis.WARNING} {interaction.user.mention}: You are not the owner of this voice channel")
            return await interaction.response.send_message(embed=embed, ephemeral=True)
        await interaction.user.voice.channel.set_permissions(member, connect=True, view_channel=True)
        embed = discord.Embed(color=colors.SUCCESS, description=f"{emojis.APPROVE} {interaction.user.mention}: {member.mention} is allowed to join {interaction.user.voice.channel.mention}")
        return await interaction.response.send_message(embed=embed)

@bot.tree.context_menu(name="Voice Reject")
async def voice_reject(interaction: discord.Interaction, member: discord.Member):
    check = await interaction.client.db.fetchrow("SELECT * FROM voicemaster WHERE guild_id = $1", interaction.guild.id)
    if check is not None:
        channel = interaction.guild.get_channel(check["channel_id"])
        if channel is None:
        ...
        embed = discord.Embed(color=colors.SUCCESS, description=f"{emojis.APPROVE} {interaction.user.mention}: {member.mention} is not allowed to join {interaction.user.voice.channel.mention} anymore")
        return await interaction.response.send_message(embed=embed)

@bot.tree.context_menu(name="Accept Suggestion")
async def accept_suggestion_command(interaction: discord.Interaction, message: discord.Message):
    """Accept suggestion via context menu"""
    cog = interaction.client.get_cog("Suggestion")
    if cog:
        await cog.handle_decision(interaction, message, "Accept")
    else:
        await interaction.response.send_message("Suggestion cog not found.", ephemeral=True)

@bot.tree.context_menu(name="Reject Suggestion")
async def reject_suggestion_command(interaction: discord.Interaction, message: discord.Message):
    """Reject suggestion via context menu"""
    cog = interaction.client.get_cog("Suggestion")
    if cog:
        await cog.handle_decision(interaction, message, "Reject")
    else:
        await interaction.response.send_message("Suggestion cog not found.", ephemeral=True)
...
async def translate_command(interaction: discord.Interaction, message: discord.Message):
    ...
    language_data = await interaction.client.db.fetchrow("SELECT * FROM language_translate WHERE user_id = $1", interaction.user.id)
    if not language_data:
        return await interaction.warn("You need to set a language first.\n> Use `;language translate` to set your language")
    if message.content == "":
        return await interaction.warn("This message is empty")
    try:
        translated = GoogleTranslator(source="auto", target=f"{language_data['lang']}").translate(message.content)
        embed = discord.Embed(color=colors.NEUTRAL, title=f"Translated to {language_data['lang']}", description=f"```{translated}```")
        return await interaction.response.send_message(embed=embed, ephemeral=language_data['ephemeral'])
    except Exception:
        return await interaction.warn("This language is **not** supported.\n> Use **language codes** like `en`, `de`, or `fr`.")
                       
@bot.tree.context_menu(name="Make a Quote")
async def quote_command(interaction: discord.Interaction, message: discord.Message):
    """Create a quote out of a user's message"""
    if not bot or not bot.db:
        return
    image_binary = await interaction.client.misc.create_quote_image(
        message.author.display_avatar.url, 
        message.content, 
        bw=False, 
        name=message.author.name
    )
    quote_channel_id = await bot.db.fetchval("SELECT channel_id FROM quotes WHERE guild_id = $1", message.guild.id)
    quote_channel = message.guild.get_channel(quote_channel_id) if quote_channel_id else None
    if quote_channel:
        await quote_channel.send(file=discord.File(fp=image_binary, filename="quote.png"))
        await interaction.response.send_message(f"The quote has been sent to {quote_channel.mention}.", ephemeral=True)
    else:
...
async def setup(bot: Evelina) -> None:
    ...
    bot.tree.add_command(user_avatar)
    bot.tree.add_command(user_banner)
    bot.tree.add_command(user_info)
    bot.tree.add_command(voice_permit)
    bot.tree.add_command(voice_reject)
    bot.tree.add_command(accept_suggestion_command)
    bot.tree.add_command(reject_suggestion_command)
