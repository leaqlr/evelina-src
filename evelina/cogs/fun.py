...
class Fun(Cog):
    ...
    @command(name="rockpaperscissors", aliases=["rps"], usage="rockpaperscissors accurs", description="Play rock-paper-scissors with a member or the bot")
    ...
    @command(name="tictactoe", aliases=["ttt"], usage="tictactoe accurs", description="Play tictactoe with a member")
    ...
    async def flags(self, ctx: EvelinaContext, difficulty: str = "easy", mode: str = "solo"):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get('https://cdn.yourbot.bot/flags.json') as resp:
                if resp.status == 200:
        ...
        embed.set_footer(icon_url="http://cdn.yourbot.bot/flag.png", text=f'Time: 10 Seconds ・ Difficulties: "easy", "hard", "extreme"')
    ...
    @stats.command(name="tictactoe", aliases=["ttt"], usage="stats tictactoe accurs", description="View a member's stats for tictactoe")
    async def stats_ttt(self, ctx: EvelinaContext, *, user: User = Author):
        await self.stats_execute(ctx, user)

    @stats.command(name="blacktea", aliases=["bt"], usage="stats blacktea accurs", description="View a member's stats for blacktea")
    async def stats_blacktea(self, ctx: EvelinaContext, *, user: User = Author):
        await self.stats_execute(ctx, user)

    @stats.command(name="rockpaperscissors", aliases=["rps"], usage="stats rockpaperscissors accurs", description="View a member's stats for rockpaperscissors")
    async def stats_rps(self, ctx: EvelinaContext, *, user: User = Author):
        await self.stats_execute(ctx, user)

    @stats.command(name="flags", usage="stats flags accurs", description="View a member's stats for flags")
    ...
    async def quran(self, ctx: EvelinaContext):
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/quran?key={config.EVELINA}")
        all_ayahs = []
    ...
    async def dog(self, ctx: EvelinaContext):
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/dog?key={config.EVELINA}")
    ...
    async def cat(self, ctx: EvelinaContext):
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/cat?key={config.EVELINA}")
        if isinstance(result, list):
            result = result[0]
    ...
    async def bird(self, ctx: EvelinaContext):
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/bird?key={config.EVELINA}")
        if isinstance(result, list):
            result = result[0]
        embed = Embed(color=colors.NEUTRAL)
    ...
    async def capybara(self, ctx: EvelinaContext):
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/capybara?key={config.EVELINA}")
        if isinstance(result, list):
            result = result[0]
        embed = Embed(color=colors.NEUTRAL)
    ...
    async def meme(self, ctx: EvelinaContext):
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/meme?key={config.EVELINA}")
        if isinstance(result, list):
            result = result[0]
        embed = Embed(color=colors.NEUTRAL)
        embed.set_image(url=result["url"])
    ...
    async def dadjoke(self, ctx: EvelinaContext):
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/dadjoke?key={config.EVELINA}")
    ...
    async def advice(self, ctx: EvelinaContext):
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/advice?key={config.EVELINA}")
    ...
    async def uselessfact(self, ctx: EvelinaContext):
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/uselessfact?key={config.EVELINA}")
        if isinstance(result, list):
            result = result[0]
    ...
    async def rizz(self, ctx: EvelinaContext):
        result = await self.bot.session.get_json(f"https://api.yourbotbot/fun/rizz?key={config.EVELINA}")
        if isinstance(result, list):
            result = result[0]
        return await ctx.evelina_send(f"{result['text']}")

    @command(name="gay", usage="gay accurs", description="Gay rate yourself or a given member")
    ...
    @command(name="furry", usage="furry accurs", description="Furry rate yourself or a given member")
    ...
    @command(name="pp", usage="pp accurs", description="Check pp size from yourself or a given member")
    ...
    @command(name="customquote", aliases=["cq"], usage="customquote accurs Hello world", description="Create a custom quote")
    ...
    @command(name="tweet", usage="tweet accurs Whats up?", description="Create a tweet")
