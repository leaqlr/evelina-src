...
class Economy(Cog):
    ...
    @command(aliases=["give", "pay"], usage="transfer accurs 125", description="Transfer cash to a member")
    @create_account()
    async def transfer(self, ctx: EvelinaContext, member: EligibleEconomyMember, amount: TransferAmount):
        ...
        async with self.locks[ctx.author.id], self.locks[member.id]:
            ...
            check = await self.bot.db.fetchrow("SELECT * FROM economy_limit WHERE user_id = $1 AND date = $2", ctx.author.id, datetime.datetime.now(datetime.timezone.utc).date())
            if check:
                limit = 10000000 - check['spent']
            else:
                limit = 10000000
            if amount > limit:
                return await ctx.send_warning(f"You reached daily transfer limit. You can transfer **{self.bot.misc.humanize_number(limit)}** {self.cash} today!")
            yes_callback = partial(self.yes_callback, ctx.author.id, member.id, amount)
            no_callback = partial(self.no_callback, ctx.author.id)
            return await ctx.confirmation_send(f"{emojis.QUESTION} {ctx.author.mention}: Are you sure you want to transfer **{self.bot.misc.humanize_number(amount)}** {self.cash} to {member.mention}?\n> **Important:** When transferring money, a tax of 25% is charged.", yes_callback, no_callback)
    async def yes_callback(self, sender_id, receiver_id, amount, interaction: Interaction):
        async with self.locks[sender_id], self.locks[receiver_id]:
            ...
            if amount <= 0:
                return await interaction.response.edit_message(embed=Embed(description=f"{emojis.WARNING} You can't transfer **0** 💳", color=colors.WARNING), view=None)
            if sender_data["card"] < amount:
                return await interaction.response.edit_message(embed=Embed(description=f"{emojis.WARNING} You don't have enough money on your **card** to transfer", color=colors.WARNING), view=None)
            if receiver_data["card"] + amount > receiver_data["item_bank"]:
                available_space = receiver_data["item_bank"] - receiver_data["card"]
                return await interaction.response.edit_message(embed=Embed(description=f"{emojis.WARNING} {interaction.user.mention} can only accept **{self.bot.misc.humanize_number(available_space)}** {self.cash} more due to the bank limit.", color=colors.WARNING), view=None)
            await self.bot.db.execute("UPDATE economy SET card = $1 WHERE user_id = $2", round(sender_data["card"] - amount, 2), sender_id)
            await self.bot.db.execute("UPDATE economy SET card = $1 WHERE user_id = $2", round(receiver_data["card"] + (amount * 0.75), 2), receiver_id)
            await self.bot.db.execute("INSERT INTO economy_limit (user_id, spent, date) VALUES ($1, $2, $3) ON CONFLICT (user_id, date) DO UPDATE SET spent = economy_limit.spent + EXCLUDED.spent", interaction.user.id, amount, datetime.datetime.now(datetime.timezone.utc).date())
            ...
            await interaction.response.edit_message(embed=Embed(description=f"{emojis.APPROVE} {interaction.user.mention}: Successfully transferred **{self.bot.misc.humanize_number(amount)}** {self.cash} (`{self.bot.misc.humanize_number(amount * 0.75)}`) to <@{receiver_id}>", color=colors.SUCCESS), view=None)
    ...
    @command(aliases=["bal"], usage="balance accurs", description="Check someone's balance and rank")
    @create_account()
    ...
    async def guildleaderboard(self, ctx: EvelinaContext):
        ...
        results = await self.bot.db.fetch("SELECT user_id, cash, card FROM economy")
        if not results:
            return await ctx.send_warning("No one has any cash yet.")
        server_members = {member.id for member in ctx.guild.members}
        sorted_results = sorted(
            (result for result in results if result['user_id'] in server_members),
            key=lambda c: c["cash"] + c["card"], reverse=True
        )
        to_show = [
    ...
    async def roulette(self, ctx: EvelinaContext, amount: CashAmount, bet: BetConverter):
        ...
        async with self.locks.setdefault(ctx.author.id, asyncio.Lock()):
            ...
            data = await self.bot.session.get_json(
                f"https://api.yourbotbot/roulette?bet={bet}&amount={amount}&key={config.EVELINA}"
            )
    ...
    async def mines(self, ctx: EvelinaContext, amount: CashAmount, count: int = 4):
        ...
        if amount < 10:
            return await ctx.send_warning(f"You can't bet less than **10** {self.cash}")
        protected_ids = [206832952980668428, 335500798752456705] #[accurs, welovecatcher]
        grid_size = 4
    ...
    @command(name="rob", usage="rob accurs", description="Attempt to steal money from another player")
    @create_account()
    @rob_taken()
    @cooldown(1, 5, BucketType.user)
    ...
    @command(aliases=["cd"], usage="cooldowns accurs", description="Check the cooldowns for all economy commands")
    @create_account()
    ...
    @card.command(name="sell", usage="card sell 1 accurs 1000", cooldown=5, description="Sell a card from your account")
    @create_account()
    @cooldown(1, 5, BucketType.user)
    ...
    @card.command(name="trade", usage="card trade 1 2 accurs", cooldown=5, description="Trade a card with another user")
    @create_account()
    @cooldown(1, 5, BucketType.user)
    ...
    @card.command(name="list", aliases=["view"], usage="card list accurs", description="View all your active cards with filters and pagination")
    @create_account()
    ...
    @card.command(name="generate", usage="card generate accurs 1000 pink", cooldown=5, description="Generate a card")
    @create_account()
    @is_manager()
    @cooldown(1, 10, BucketType.user)
    async def card_generate(self, ctx: EvelinaContext, user: User, card_id: int, type: str):
        ...
        if upload_res:
            file_url = f"https://cdn.yourbot.bot/card/{file_name}"
        else:
    ...
    @business.command(name="info", aliases=["status"], usage="business info accurs", description="Get information about your business")
    @create_account()
    ...
    @business.command(name="add", brief="bot helper", usage="business add accurs 1", description="Add a business to a user")
    @create_account()
    @is_moderator()
    ...
    @business.command(name="remove", brief="bot helper", usage="business remove accurs", description="Remove a business from a user")
    @create_account()
    @is_moderator()
    ...
    async def lab_upgrade(self, ctx: EvelinaContext):
        ...
        if upgrade_state >= 15:
            _, used_card_stars, _, _ = await self.economy.get_used_card(ctx.author.id, "lab")
            if not used_card_stars:
                return await ctx.send_warning("You **need** a card to upgrade your lab past level 15")
            if upgrade_state in range(15, 19):
                if used_card_stars < 2:
                    return await ctx.send_warning("You **need** a card with **2** stars or more to upgrade your lab past level 15")
            if upgrade_state in range(20, 29):
                if used_card_stars < 3:
                    return await ctx.send_warning("You **need** a card with **3** stars or more to upgrade your lab past level 20")
            if upgrade_state in range(30, 39):
                if used_card_stars < 4:
                    return await ctx.send_warning("You **need** a card with **4** stars or more to upgrade your lab past level 30")
            if upgrade_state in range(40, 49):
                if used_card_stars < 5:
                    return await ctx.send_warning("You **need** a card with **5** stars or more to upgrade your lab past level 40")
            if upgrade_state >= 50:
        ...
        if check["cash"] < next_upgrade_cost:
            return await ctx.send_warning(f"You **do not** have enough cash to upgrade\nYou need **{self.bot.misc.humanize_number(next_upgrade_cost)}** {self.cash}")
        ...
        return await ctx.send_success(f"You've successfully upgraded your lab to level **{upgrade_state + 1}**\n> The total upgrade cost was **{self.bot.misc.humanize_number(next_upgrade_cost)}** {self.cash}")
    
    @lab.command(name="info", aliases=["status"], usage="lab status accurs")
    @create_account()
    ...
    async def lab_collect(self, ctx: EvelinaContext):
        ...
        if upgrade_state >= 16:
            _, used_card_stars, _, _ = await self.economy.get_used_card(ctx.author.id, "lab")
            if not used_card_stars:
                return await ctx.send_warning("You can't collect your earnings without a card")
            if upgrade_state in range(16, 20):
                if used_card_stars < 2:
                    return await ctx.send_warning("You **need** a card with **2** stars or more to collect your lab earnings past level 10")
            if upgrade_state in range(21, 30):
                if used_card_stars < 3:
                    return await ctx.send_warning("You **need** a card with **3** stars or more to collect your lab earnings past level 20")
            if upgrade_state in range(31, 40):
                if used_card_stars < 4:
                    return await ctx.send_warning("You **need** a card with **4** stars or more to collect your lab earnings past level 30")
            if upgrade_state in range(41, 50):
                if used_card_stars < 5:
                    return await ctx.send_warning("You **need** a card with **5** stars or more to collect your lab earnings past level 40")
        if time_passed < 3600:
            next_collect_time = last_collected + 3600
        ...
        return await ctx.send_success(f"You've successfully collected **{self.bot.misc.humanize_number(earnings)}** {self.cash}\nNext collection time: {next_collect_time_str}")

    @lab.command(name="add", brief="bot moderator", usage="lab add accurs 1", description="Add a laboratory to a user")
    @create_account()
    @is_moderator()
    ...
    @lab.command(name="remove", brief="bot moderator", usage="lab remove accurs", description="Remove a laboratory from a user")
    @create_account()
    @is_moderator()
    ...
    @command(name="networth", aliases=["nw"], usage="networth accurs")
    @create_account()
    ...
    @company.command(name="transfer", usage="company transfer accurs")
    @create_account()
    ...
    @company.command(name="userinfo", aliases=["ui", "user"], usage="company userinfo accurs")
    @create_account()
    ...
    async def company_list(self, ctx: EvelinaContext):
        ...
        for i in range(0, len(company_entries), entries_per_page):
            page_number = (i // entries_per_page) + 1
            embed = Embed(title=f"Companys", color=colors.NEUTRAL)
    ...
    async def company_icon(self, ctx: EvelinaContext, icon: str = None):
        ...
        if ctx.message.attachments:
            ...
            if response:
                ...
                file_url = f"https://cdn.yourbot.bot/company/{file_name}"
    ...
    @company.command(name="kick", usage="company kick accurs")
    @create_account()
    ...
    @company.command(name="uprank", usage="company uprank accurs")
    @create_account()
    ...
    @company.command(name="downrank", usage="company downrank accurs")
    @create_account()
    ...
    @company.command(name="invite", usage="company invite accurs")
    @create_account()
    ...
    @company.command(name="uninvite", usage="company uninvite accurs")
    @create_account()
    ...
    @company_vault.command(name="bonus", usage="company vault bonus accurs 1000000")
    @create_account()
